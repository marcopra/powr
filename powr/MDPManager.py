...
class MDPManager:
    ...
    def update_Q(self):
        ...
        f_tmp_Q = V.T @ jnp.linalg.solve(V @ f_big_M @ V.T, V @ self.FTL.r)
    ...
    def collect_data(self, n_episodes=1):
        ...
        for episode_id in range(n_episodes):
            ...
            while True:
                ...
                if new_terminated_mask.any():
                    ...
                    f_X = jnp.vstack((f_X, new_states[new_terminated_mask]))
                    f_Y_transitions = jnp.vstack((f_Y_transitions, new_states[new_terminated_mask]))
                ...
                if f_X is None:  # First iteration
                    f_X = states[non_terminated_mask]
                    f_Y_transitions = new_states[non_terminated_mask]
                    f_Y_rewards = rewards[non_terminated_mask] 
                    f_A = actions[non_terminated_mask]
                else:  # After first iteration, concatenate
                    f_X = jnp.vstack((f_X, states[non_terminated_mask]))
                    f_Y_transitions = jnp.vstack((f_Y_transitions, new_states[non_terminated_mask]))
        ...
        self.FTL.collect_data(f_A, f_X, f_Y_transitions, f_Y_rewards.reshape(-1, 1), cum_rewards.flatten().mean() >= self.env.spec.reward_threshold, seed = self.seed, reward = cum_rewards.flatten().mean())

        self._DATA_COLLECTED_BUT_NOT_TRAINED = True

        return cum_rewards.flatten().mean(), total_timesteps

    def test(self, n_episodes=1, plot=False, path=None, wandb_log=False, current_epoch=None):
    ...
    def __setstate__(self, state):
        ...
        self.FTL = IncrementalRLS(kernel=self.kernel, n_actions=self.n_actions, la=self.la, n_subsamples=self.n_subsamples)
